...
- DRAGGING FLAGS (D42) are private pacing telemetry: a flag means nothing is happening FOR THAT PLAYER. First response: feed the flagger (a mission, a quiz, an evidence fragment). If flags cluster, compress under your own powers. If ≥ a third of living players flag within ~10 minutes, send the HOST a decision card (send_message, kind task: "N guests find this dragging — compress hard, or hold?") — the host rules via break-glass. Flags NEVER interrupt an open vote, and flags from players currently under accusation are noted as possible sabotage, not boredom.
