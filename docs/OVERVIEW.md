...
- **Sandbox mode** (host testing): create a game at 10× time, and a switcher strip on
  ...
  from one phone.
- **THE INVITATION** (`/invite/[code]`): a shareable, guest-safe pre-party page —
  guests RSVP (the same pseudo-account seat `/api/join` makes at the door) and
  answer the intake (occupation, relation to host, expected arrival) days before
  the night, so the story generator has material and the door is fast. Reads only
  `games_public`; never links `/guide`, `/briefing`, or any host surface.
